...
MIN_Loss_Cal::MIN_Loss_Cal(
        vector<int> &radix_arg, string switch_type_arg, string stage_switch_type_arg) :
        switch_type(switch_type_arg), stage_switch_type(stage_switch_type_arg),
        ...
        max_num_crossing_out(0)
{
    ...
    result_filename0 = to_string(port) + "port_" + switch_type + "_" + p_stage_switches[0]->type;
...
void MIN_Loss_Cal::sweep_all_loss(SimParaClass &sim_para)
{
    double loss_i;
    PathClass path;
    for (int s = 0; s < port; ++s)
...
void MIN_Loss_Cal::write_result()
{
...
double Baseline_Loss_Cal::cal_loss(int s, int d, PathClass &path, SimParaClass &sim_para)
{
    // s = 1039;
    // d = 0;
...
double Baseline_Loss_Cal::cal_loss(vector<int> &ss_index_wobias,
                                   PathClass &path,
...
void Baseline_Loss_Cal::get_path(int s, int d, PathClass &path, vector<int> &ss_index_wobias)
{
    int residual_s = s;
    int residual_d = d;
    int bias = 0;
